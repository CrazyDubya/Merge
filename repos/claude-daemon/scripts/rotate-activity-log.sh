#!/bin/bash
#
# Log Rotation Script for activity.log
#
# Purpose: Rotate the activity log when it exceeds size threshold
# Author: Maintainer persona
# Date: 2025-12-25
#
# Usage:
#   ./rotate-activity-log.sh              # Check size and rotate if needed
#   ./rotate-activity-log.sh --force      # Force rotation regardless of size
#   ./rotate-activity-log.sh --dry-run    # Show what would happen without doing it
#
# Safety Features:
...
#
# Recovery:
#   Archives are stored in logs/archives/ with timestamps
...
TEMP_DIR="/tmp/activity-log-rotation-$$"
...
count_existing_rotations() {
    if [ ! -d "$ARCHIVE_DIR" ]; then
        echo "0"
        return
    fi
    find "$ARCHIVE_DIR" -name "activity-log-*.log.gz" 2>/dev/null | wc -l
}
...
cleanup_old_rotations() {
    ...
    rotation_count=$(find "$ARCHIVE_DIR" -name "activity-log-*.log.gz" 2>/dev/null | wc -l)
    ...
    find "$ARCHIVE_DIR" -name "activity-log-*.log.gz" -type f | sort | head -n "$to_delete" | while read -r file; do
...
perform_rotation() {
    ...
    local archive_base="activity-log-${timestamp}.log"
    local archive_compressed="activity-log-${timestamp}.log.gz"
    ...
    if ! gunzip -t "$archive_path" 2>/dev/null; then
        ...
        log_error "Backup preserved at: ${TEMP_DIR}/backup.log"
        return 1
    fi
...
This directory contains archived activity log entries. Each archive is compressed with gzip.
...
main() {
    ...
    log_info "===== Activity Log Rotation Script ====="

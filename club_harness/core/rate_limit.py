"""Rate limiting utilities (token bucket + quota), ported from Village.

Ported from Village village/utils/rate_limiter.py via git subtree merge;
see docs/PROVENANCE.md. Added RateLimiter.acquire_nowait(): a sync,
best-effort helper so club_harness Agent.chat() can consult a limiter
...
class RateLimiter:
    ...
    def acquire_nowait(self, tokens: int = 1) -> bool:
        """Synchronously try to acquire tokens without waiting or locking.

        Best-effort helper for sync call sites (e.g. Agent.chat()). For
        contended use, prefer the async acquire()/try_acquire().
        """
        now = time.monotonic()
        elapsed = now - self.last_refill
        self.tokens = min(
...
class QuotaManager:
    ...
    async def check_quota(self, amount: int = 1) -> bool:
        ...
        """
        async with self._lock:
            return self._check_quota_locked(amount)

    def _check_quota_locked(self, amount: int = 1) -> bool:
        """Quota check assuming the lock is already held.

        Split out because use_quota() must check-then-record atomically;
        calling check_quota() from inside use_quota() would deadlock on the
        non-reentrant asyncio lock (bug in the original Village code, fixed
        in this port).
        """
        now = datetime.utcnow()
    ...
    async def use_quota(self, amount: int = 1) -> bool:
        ...
        async with self._lock:
            ...
            # Check if quota is available (lock already held - do not call
            # check_quota() here; it would deadlock on the asyncio lock)
            if not self._check_quota_locked(amount):
                return False

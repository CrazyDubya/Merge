"""Unit tests for ported Village LLM providers.

Ported from Village tests/unit/test_providers.py via git subtree merge;
see docs/PROVENANCE.md. Import paths rewritten village.* -> club_harness.*.
"""


import pytest
from unittest.mock import AsyncMock, MagicMock, patch
from club_harness.core.errors import LLMError as LLMProviderError
...
class TestOpenAIProvider:
    ...
    async def test_openai_generate(self, mock_openai_response):
        ...
        with patch('club_harness.llm.providers.openai.OPENAI_AVAILABLE', True):
            with patch('club_harness.llm.providers.openai.AsyncOpenAI') as mock_client_class:
                from club_harness.llm.providers.openai import OpenAIProvider
...
class TestAnthropicProvider:
    ...
    async def test_anthropic_generate(self, mock_anthropic_response):
        ...
        with patch('club_harness.llm.providers.anthropic.ANTHROPIC_AVAILABLE', True):
            with patch('club_harness.llm.providers.anthropic.AsyncAnthropic') as mock_client_class:
                from club_harness.llm.providers.anthropic import AnthropicProvider
...
class TestGoogleProvider:
    ...
    async def test_google_generate(self, mock_google_response):
        ...
        with patch('club_harness.llm.providers.google.GOOGLE_AVAILABLE', True):
            with patch('club_harness.llm.providers.google.genai') as mock_genai:
                from club_harness.llm.providers.google import GoogleProvider
...
class TestRateLimiter:
    ...
    async def test_rate_limiter_acquire(self):
        ...
        from club_harness.core.rate_limit import RateLimiter
    ...
    async def test_quota_manager_check_quota(self):
        """Test quota checking."""
        from club_harness.core.rate_limit import QuotaManager
...
class TestStorage:
    ...
    async def test_memory_storage(self):
        ...
        from club_harness.memory.stores import InMemoryStorage
...
class TestMetrics:
    ...
    @pytest.mark.skip(reason="Village metrics module not ported (Prometheus optional)")
